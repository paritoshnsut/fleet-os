...
import {
  ...
  Calculator, TrendingUp, BarChart2, Zap, Download, SlidersHorizontal, Loader2,
} from 'lucide-react';
...
export default function TCOAnalysis() {
  ...
  const handleDownloadPDF = async () => {
    setIsPrinting(true);
    await new Promise(r => setTimeout(r, 400)); // let accordions expand
    ...
    try {
      ...
      const ML        = 14;
      const MR        = 14;
      const INNER     = CW - ML - MR;
      const today     = new Date().toLocaleDateString('en-IN', { day: 'numeric', month: 'long', year: 'numeric' });

      const addPageHeader = (sub = '') => {
        doc.setFillColor(15, 23, 42); doc.rect(0, 0, CW, 14, 'F');
        doc.setFontSize(9); doc.setFont('helvetica', 'bold'); doc.setTextColor(255, 255, 255);
        doc.text('FleetOS', ML, 9);
        doc.setFont('helvetica', 'normal'); doc.setTextColor(148, 163, 184);
        if (sub) doc.text(sub, CW / 2, 9, { align: 'center' });
        doc.text(today, CW - MR, 9, { align: 'right' });
        doc.setFillColor(59, 130, 246); doc.rect(0, 14, CW, 0.5, 'F');
      };

      const addPageFooter = (n) => {
        doc.setFontSize(8); doc.setTextColor(148, 163, 184);
        doc.text('CONFIDENTIAL · Generated by FleetOS · Tata Motors CV Passenger Division', CW / 2, 287, { align: 'center' });
        doc.text(`FleetOS · Tata Motors CV · Page ${n}`, CW / 2, 292, { align: 'center' });
      };

      // ── PAGE 1: COVER ─────────────────────────────────────────────────────
      doc.setFillColor(15, 23, 42); doc.rect(0, 0, CW, 65, 'F');
      doc.setFillColor(59, 130, 246); doc.roundedRect(ML, 14, 12, 12, 2, 2, 'F');
      doc.setFontSize(9); doc.setFont('helvetica', 'bold'); doc.setTextColor(255, 255, 255);
      doc.text('F', ML + 6, 21.5, { align: 'center' });
      doc.setFontSize(14); doc.text('FleetOS', ML + 16, 21);
      doc.setFont('helvetica', 'normal'); doc.setFontSize(8); doc.setTextColor(148, 163, 184);
      doc.text('Tata Motors · CV Passenger Division · Pune', ML + 16, 27);
      doc.setFillColor(59, 130, 246); doc.rect(0, 65, CW, 1.5, 'F');

      doc.setTextColor(15, 23, 42); doc.setFont('helvetica', 'bold'); doc.setFontSize(28);
      doc.text('TCO Cost Analysis', ML, 92);
      doc.text('Report', ML, 106);
      doc.setFont('helvetica', 'normal'); doc.setFontSize(11); doc.setTextColor(100, 116, 139);
      doc.text(`${busType} Bus — ${busType === 'EV' ? 'Electric' : 'Diesel'} lifecycle modelling`, ML, 118);
      ...
      autoTable(doc, {
        ...
        body: [
          ['Report date',         today],
          ['Bus type',            busType === 'EV' ? 'Electric Bus' : 'Diesel Bus'],
          ['Analysis period',     `${params.busLife} years`],
          ['ECP (vehicle cost)',  `₹${(params.ecp / 100000).toFixed(1)} Lakh`],
          ['km per day',          `${params.kmPerDay} km`],
          ['Operational days',    `${params.operationalDays} days/yr`],
          [busType === 'EV' ? 'Energy efficiency' : 'Fuel efficiency',
           busType === 'EV' ? `${params.fuelEfficiency} kWh/km` : `${params.fuelEfficiency} km/L`],
          [busType === 'EV' ? 'Power tariff' : 'Diesel price',
           `₹${params.fuelTariff}/${busType === 'EV' ? 'kWh' : 'L'}`],
        ],
        theme: 'plain',
        styles: { fontSize: 10, cellPadding: { top: 3, bottom: 3, left: 0, right: 4 } },
        columnStyles: {
          0: { textColor: [148, 163, 184], cellWidth: 55 },
      ...
      // ── PAGE 2: SUMMARY & EAP BREAKDOWN ──────────────────────────────────
      doc.addPage();
      addPageHeader(`TCO Analysis · ${busType} Bus`);
      let y = 22;
      ...
      const cols3 = INNER / 3;
      [
        { label: 'EV Total EAP',  value: `₹${results.totalEAP.toFixed(2)}/km`, color: [59, 130, 246] },
        { label: 'DSL Total EAP', value: dslResults ? `₹${dslResults.totalEAP.toFixed(2)}/km` : '—', color: [249, 115, 22] },
        {
          label: dslResults
            ? (results.totalEAP < dslResults.totalEAP ? '✓ EV Saves' : '✗ EV Costs More')
            : 'Annual km',
          value: dslResults
            ? `₹${Math.abs(results.totalEAP - dslResults.totalEAP).toFixed(2)}/km`
            : `${results.annualKm.toLocaleString('en-IN')} km/yr`,
          color: dslResults
            ? (results.totalEAP < dslResults.totalEAP ? [34, 197, 94] : [239, 68, 68])
            : [100, 116, 139],
        },
      ].forEach((m, i) => {
        const bx = ML + i * (cols3 + 2);
        doc.setFillColor(248, 250, 252); doc.roundedRect(bx, y, cols3, 22, 2, 2, 'F');
        doc.setDrawColor(226, 232, 240); doc.roundedRect(bx, y, cols3, 22, 2, 2, 'S');
        doc.setFontSize(7); doc.setFont('helvetica', 'normal'); doc.setTextColor(...m.color);
        doc.text(m.label.toUpperCase(), bx + cols3 / 2, y + 5.5, { align: 'center' });
        doc.setFontSize(13); doc.setFont('helvetica', 'bold'); doc.setTextColor(15, 23, 42);
        doc.text(m.value, bx + cols3 / 2, y + 15, { align: 'center' });
      });
      y += 28;

      // Input parameters side-by-side
      doc.setFontSize(9); doc.setFont('helvetica', 'bold'); doc.setTextColor(15, 23, 42);
      doc.text('INPUT PARAMETERS', ML, y); y += 4;
      autoTable(doc, {
        startY: y,
        head: [['EV Parameter', 'Value', 'DSL Parameter', 'Value']],
        body: [
          ['ECP', `₹${(params.ecp/100000).toFixed(1)}L`, 'ECP', `₹${(dslParams.ecp/100000).toFixed(1)}L`],
          ['km/day', `${params.kmPerDay}`, 'km/day', `${dslParams.kmPerDay}`],
          ['Days/yr', `${params.operationalDays}`, 'Days/yr', `${dslParams.operationalDays}`],
          ['Efficiency', `${params.fuelEfficiency} kWh/km`, 'Efficiency', `${dslParams.fuelEfficiency} km/L`],
          ['Tariff', `₹${params.fuelTariff}/kWh`, 'Diesel price', `₹${dslParams.fuelTariff}/L`],
          ['Interest rate', `${(params.interestRate*100).toFixed(1)}%`, 'Interest rate', `${(dslParams.interestRate*100).toFixed(1)}%`],
          ['Analysis period', `${params.busLife} yrs`, 'Analysis period', `${dslParams.busLife} yrs`],
        ],
        theme: 'grid',
        headStyles: { fillColor: [15,23,42], textColor: 255, fontSize: 8, fontStyle: 'bold' },
        styles: { fontSize: 8, cellPadding: 2.5 },
        columnStyles: {
          0: { textColor: [59,130,246], fontStyle: 'bold' },
      ...
      // EAP breakdown table
      doc.setFontSize(9); doc.setFont('helvetica', 'bold'); doc.setTextColor(15,23,42);
      doc.text('COST COMPONENT BREAKDOWN (EAP ₹/km)', ML, y); y += 4;
      const eapRows = ROW_ORDER
        .filter(lbl => results.eapPerRow[lbl] && Math.abs(results.eapPerRow[lbl]) > 0.001 && lbl !== 'Battery replacement cost')
        .map(lbl => [
          lbl,
          results.eapPerRow[lbl].toFixed(2),
          dslResults?.eapPerRow[lbl] ? dslResults.eapPerRow[lbl].toFixed(2) : '—',
          `${(results.eapPerRow[lbl] / results.totalEAP * 100).toFixed(1)}%`,
        ]);
      eapRows.push(['TOTAL EAP', results.totalEAP.toFixed(2), dslResults ? dslResults.totalEAP.toFixed(2) : '—', '100%']);
      autoTable(doc, {
        startY: y,
        head: [['Cost Category', 'EV EAP ₹/km', 'DSL EAP ₹/km', 'EV Share']],
        body: eapRows,
        theme: 'striped',
        headStyles: { fillColor: [15,23,42], textColor: 255, fontSize: 8, fontStyle: 'bold' },
        styles: { fontSize: 8, cellPadding: 2.5 },
        columnStyles: { 1: { fontStyle: 'bold', textColor: [59,130,246] }, 2: { textColor: [249,115,22] } },
        didParseCell(data) {
          if (data.row.index === eapRows.length - 1) {
      ...
      // ── PAGE 3: LIFECYCLE COST TABLE ──────────────────────────────────────
      doc.addPage();
      addPageHeader(`TCO Analysis · Lifecycle Costs`);
      y = 22;
      doc.setFontSize(11); doc.setFont('helvetica', 'bold'); doc.setTextColor(15,23,42);
      doc.text(`EV LIFECYCLE COST MAPPING (₹ LAKH) — ${params.busLife}-YEAR PERIOD`, ML, y); y += 6;

      const yrLabels = Array.from({ length: years }, (_, i) => `Yr ${i+1}`);
      const lcHead   = ['Cost Category', 'YoY', ...yrLabels, 'EAP ₹/km'];
      const lcBody   = ROW_ORDER.map(lbl => {
        const costs = results.rows[lbl];
        if (!costs || (costs.every(v => v === 0) && lbl === 'Battery replacement cost')) return null;
        const yoy = lbl === 'Fuel Cost' ? params.fuelYoY
          : lbl.includes('Manpower') ? params.manpowerYoY
          : lbl.includes('AMC Cost (') ? params.amcYoY
          : lbl.includes('Misc') ? params.miscYoY : null;
        return [lbl, yoy ? `${(yoy*100).toFixed(1)}%` : '—', ...costs.map(v => fmtL(v)),
          results.eapPerRow[lbl] < 0.01 ? '—' : results.eapPerRow[lbl].toFixed(2)];
      }).filter(Boolean);
      lcBody.push(['TOTAL (₹ Lakh)', '—',
        ...Array.from({ length: years }, (_, i) => fmtL(ROW_ORDER.reduce((s, lbl) => s + (results.rows[lbl]?.[i] || 0), 0))),
      ...
      autoTable(doc, {
        ...
        columnStyles: {
          0: { cellWidth: 50 },
          [lcHead.length - 1]: { fillColor: [239,246,255], textColor: [29,78,216], fontStyle: 'bold' },
        },
        didParseCell(data) {
          if (data.row.index === lcBody.length - 1) {
            data.cell.styles.fillColor = [15,23,42];
            data.cell.styles.textColor = [255,255,255];
            data.cell.styles.fontStyle = 'bold';
          }
      ...
      // ── PAGE 4: SENSITIVITY (html2canvas) ────────────────────────────────
      let pageNum = 4;
      if (sensitivity && sensitivityRef.current) {
        const cvs = await html2canvas(sensitivityRef.current, { scale: 2, backgroundColor: '#ffffff', logging: false, useCORS: true });
        const imgH = (cvs.height / cvs.width) * INNER;
        doc.addPage();
        addPageHeader('TCO Analysis · Sensitivity');
        y = 22;
        doc.setFontSize(11); doc.setFont('helvetica', 'bold'); doc.setTextColor(15,23,42);
        doc.text('SENSITIVITY ANALYSIS (±10% PARAMETER CHANGE → % EAP IMPACT)', ML, y); y += 6;
        doc.addImage(cvs.toDataURL('image/png'), 'PNG', ML, y, INNER, Math.min(imgH, 240));
        addPageFooter(pageNum++);
      }

      // ── PAGE 5-6: PARITY HEATMAPS (html2canvas) ──────────────────────────
      const heatmapSections = [
        { ref: heatmap1Ref, title: 'PARITY TABLE 1 — kWh/km (Efficiency) vs EV ECP' },
        { ref: heatmap2Ref, title: 'PARITY TABLE 2 — km/day vs EV ECP' },
      ];
      for (const { ref, title } of heatmapSections) {
        if (!ref.current) continue;
        const cvs  = await html2canvas(ref.current, { scale: 2, backgroundColor: '#ffffff', logging: false, useCORS: true });
        const imgH = (cvs.height / cvs.width) * INNER;
        doc.addPage();
        addPageHeader('TCO Analysis · Parity Heatmap');
        y = 22;
        doc.setFontSize(10); doc.setFont('helvetica', 'bold'); doc.setTextColor(15,23,42);
        doc.text(title, ML, y); y += 5;

        const legend = [
          { bg: [20,83,45],   tx: [187,247,208], label: 'EV wins >10%' },
          { bg: [21,128,61],  tx: [220,252,231], label: 'EV wins 5-10%' },
          { bg: [134,239,172],tx: [20,83,45],    label: 'EV wins 2-5%' },
          { bg: [220,252,231],tx: [22,101,52],   label: 'EV wins <2%' },
          { bg: [254,226,226],tx: [127,29,29],   label: 'DSL wins <2%' },
          { bg: [239,68,68],  tx: [255,255,255], label: 'DSL wins >5%' },
          { bg: [127,29,29],  tx: [252,165,165], label: 'DSL wins >10%' },
        ];
        let lx = ML;
        doc.setFontSize(6.5); doc.setFont('helvetica', 'normal');
        legend.forEach(({ bg, label }) => {
          doc.setFillColor(...bg); doc.roundedRect(lx, y, 3, 3, 0.5, 0.5, 'F');
          doc.setTextColor(80,80,80); doc.text(label, lx + 4.5, y + 2.5);
          lx += doc.getTextWidth(label) + 9;
        });
        y += 7;
        doc.addImage(cvs.toDataURL('image/png'), 'PNG', ML, y, INNER, Math.min(imgH, 248));
        addPageFooter(pageNum++);
      }

      doc.save(`FleetOS_TCO_${busType}_${new Date().toISOString().slice(0,10)}.pdf`);
    } catch (err) {
      console.error('TCO PDF error:', err);
    }
    setIsPrinting(false);
  };
          ...
          <div className="flex items-center gap-2">
            ...
            {results && (
              <button
                ...
                disabled={isPrinting}
                className="flex items-center gap-1.5 px-3 py-2 text-xs font-medium bg-green-600 hover:bg-green-700 text-white rounded-lg transition-colors shadow-sm disabled:opacity-60"
              >
                {isPrinting
            ...
            <div ref={resultsRef} className="space-y-4 scroll-mt-6">
              ...
              {parityData && (
                ...
                >
                  ...
                  <div ref={heatmap1Ref} className="bg-white rounded-xl border border-slate-200 p-4">
                    <ParityHeatmap
                  ...
                  <div ref={heatmap2Ref} className="bg-white rounded-xl border border-slate-200 p-4">
                    <ParityHeatmap
              ...
              >
                ...
                {/* Sensitivity Chart */}
                {sensitivity && (
                  <div ref={sensitivityRef} className="bg-white rounded-xl border border-slate-200 p-5">
